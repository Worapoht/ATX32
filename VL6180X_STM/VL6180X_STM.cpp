...
void VL6180X_STM::setAddress(uint8_t new_addr)
{
...
void VL6180X_STM::configureDefault(void)
{
  ...
  writeReg(VL6180X_STM::SYSRANGE__MAX_CONVERGENCE_TIME, 0x31);
...
void VL6180X_STM::writeReg(uint16_t reg, uint8_t value)
{
  HWire.beginTransmission(address);
  HWire.write((reg >> 8) & 0xff);  // reg high byte
  HWire.write(reg & 0xff);         // reg low byte
  HWire.write(value);
  last_status = HWire.endTransmission();
}

// Writes a 16-bit register
void VL6180X_STM::writeReg16Bit(uint16_t reg, uint16_t value)
{
  HWire.beginTransmission(address);
  HWire.write((uint8)((reg >> 8) & 0xff));  // reg high byte
  HWire.write((uint8)(reg & 0xff));         // reg low byte
  HWire.write((uint8)((value >> 8) & 0xff));  // value high byte
  HWire.write((uint8)(value & 0xff));         // value low byte
  last_status = HWire.endTransmission();
}

// Writes a 32-bit register
void VL6180X_STM::writeReg32Bit(uint16_t reg, uint32_t value)
{
  HWire.beginTransmission(address);
  HWire.write((reg >> 8) & 0xff);  // reg high byte
  HWire.write(reg & 0xff);         // reg low byte
  HWire.write((uint8)((value >> 24) & 0xff)); // value highest byte
  HWire.write((uint8)((value >> 16) & 0xff));
  HWire.write((uint8)((value >> 8) & 0xff));
  HWire.write((uint8)(value & 0xff));         // value lowest byte
  last_status = HWire.endTransmission();
}

// Reads an 8-bit register
uint8_t VL6180X_STM::readReg(uint16_t reg)
{
  uint8_t value;
...
uint16_t VL6180X_STM::readReg16Bit(uint16_t reg)
{
  uint16_t value;
...
uint32_t VL6180X_STM::readReg32Bit(uint16_t reg)
{
  uint32_t value;
...
// Implemented using ST's VL6180X_STM API as a reference (STSW-IMG003); see
// VL6180x_UpscaleSetScaling() in vl6180x_api.c.
void VL6180X_STM::setScaling(uint8_t new_scaling)
{
  ...
  writeReg(VL6180X_STM::SYSRANGE__PART_TO_PART_RANGE_OFFSET, ptp_offset / scaling);

  // apply scaling on CrossTalkValidHeight
  writeReg(VL6180X_STM::SYSRANGE__CROSSTALK_VALID_HEIGHT, DefaultCrosstalkValidHeight / scaling);

  // This function does not apply scaling to RANGE_IGNORE_VALID_HEIGHT.

  // enable early convergence estimate only at 1x scaling
  uint8_t rce = readReg(VL6180X_STM::SYSRANGE__RANGE_CHECK_ENABLES);
  writeReg(VL6180X_STM::SYSRANGE__RANGE_CHECK_ENABLES, (rce & 0xFE) | (scaling == 1));
}

// Performs a single-shot ranging measurement
uint8_t VL6180X_STM::readRangeSingle()
{
...
uint16_t VL6180X_STM::readAmbientSingle()
{
...
// for details.
void VL6180X_STM::startRangeContinuous(uint16_t period)
{
...
// measurement. See section 2.4.4 ("Continuous mode limits") in the datasheet
// for details.
void VL6180X_STM::startAmbientContinuous(uint16_t period)
{
  int16_t period_reg = (int16_t)(period / 10) - 1;
  period_reg = constrain(period_reg, 0, 254);
...
// for details.
void VL6180X_STM::startInterleavedContinuous(uint16_t period)
{
  int16_t period_reg = (int16_t)(period / 10) - 1;
  period_reg = constrain(period_reg, 0, 254);
...
void VL6180X_STM::stopContinuous()
{
...
// range measurement)
uint8_t VL6180X_STM::readRangeContinuous()
{
...
uint16_t VL6180X_STM::readAmbientContinuous()
{
  uint16_t millis_start = millis();
...
// timeoutOccurred()?
bool VL6180X_STM::timeoutOccurred()
{
